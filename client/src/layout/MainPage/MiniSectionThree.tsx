...
export default function ImageSlider() {
  ...
  return (
    <div
      className="relative mt-5 mb-10 mx-auto  rounded-full sm:w-[97%] w-[90%] sm:h-[200px] h-[100px] overflow-hidden"
      style={{ height: 200 }}
    >
      ...
      {images.map((src, index) => (
        ...
        {words.map((word,index)=>(
          <div key={index} className={`absolute inset-0 transition-opacity duration-1000 ${index===currentWord ? "opacity-100" : "opacity-0"}`}>
                <div className="relative  h-[200px]"> {/** to place a text over an image */}
                    <Image
            src={src}
            alt={`Slide ${index + 1}`}
            fill
            ...
            loading="eager"
          />
          <div className="absolute  top-1/2 sm:left-10 left-5 transform-translate-x-1/2 -translate-y-1/2 text-white text-center">
           <p className={`sm:text-4xl text-2xl pt-20 text-white ${josefinFont.className}`}>{word}...</p>
          </div>
        </div>
          </div>
        ))}
