...
export default function SectionThree() {
  ...
  return (
    <div className="sm:ml-0 ml-0 py-10 bg-white sm:w-screen w-full ">
                  ...
                  {isMobile ? (
                    <div className="flex justify-center px-4 ml-6">
                      {carsForThisSlide.map((car) => (
                        <div
                          key={car.id}
                          className="bg-white border border-lightSky cursor-pointer rounded-lg shadow-md overflow-hidden hover:shadow-lg transition duration-300 w-full max-w-[350px]"
                        >
                          ...
                          <div className="pt-1 px-3">
                            <h3
                              className={`text-md  text-navyBlue text-bold ${josefinFont}`}
                            >
                              {car.model} -{car.year}
                            </h3>
                            <hr className="my-1 pt-1 text-lightSky"/>
                            <div className={`grid grid-cols-3 gap-1`}>
<div className=" mx-auto"><SlSpeedometer  className="text-yellow-500 text-md"/><h6 className={`text-xs -ml-2`}>{car.mileage.toLocaleString()}km</h6></div>
...
<div className=" mx-auto"><GiGearStickPattern  className="text-green-500 text-md"/><h6 className={`text-xs -ml-2`}>Automatic</h6></div>
                            ...
                            <hr className="my-1 pt-1 text-lightSky"/>
                            <div
                              className={`flex justify-between pt-1 text-sm text-navyBlue ${josefinFont}`}
                            >
                              <p
                                className={`text-sm font-bold text-navyBlue ${josefinFont}`}
                              >
                    ...
                    <div className="flex justify-center gap-4 px-6">
                      {carsForThisSlide.map((car) => (
                        <div
                          key={car.id}
                          className="bg-white border border-lightSky cursor-pointer rounded-lg shadow-md overflow-hidden hover:shadow-lg transition duration-300 w-68 h-80 "
                        >
                          <Link key={car.id} href={`/cars/${car.id}`}>
                            <div className="relative h-52">
                              <Image
                          ...
                          <div className="pt-1 px-3">
                            <h3
                              className={`text-sm text-navyBlue text-bold ${josefinFont}`}
                            >
                              {car.model} -{car.year}
                            </h3>
                            <hr className="my-1 pt-1 text-lightSky"/>
                            ...
                            <hr className="my-1 pt-1 text-lightSky"/>
                            <div
                              className={`flex justify-between pt-1 ${josefinFont}`}
                            >
                              <p
                                className={`text-sm font-bold text-navyBlue ${josefinFont}`}
                              >
